...
class HotshotXLOutput:

    working_dir = "HotshotXl"
    def output(
            self, p: StableDiffusionProcessing, res: Processed, params: HotshotXLParams
    ):
        video_paths = []
        logger.info("Merging images into GIF.")
        Path(f"{p.outpath_samples}/{self.working_dir}").mkdir(exist_ok=True, parents=True)
        ...
        for i in range(res.index_of_first_image, len(res.images), step):
            ...
            seq = images.get_next_sequence_number(
                f"{p.outpath_samples}/{self.working_dir}", ""
            )
    ...
    def _add_reverse(self, params: HotshotXLParams, video_list: list):
    ...
    def _interp(
        ...
        tmp_folder = f"{p.outpath_samples}/{self.working_dir}/tmp"
        ...
        if "PNG" in params.format:
            save_interp_path = f"{p.outpath_samples}/{self.working_dir}/interp"
    ...
    def _save(
        ...
        if "GIF" in params.format:
            ...
            if shared.opts.data.get("hotshot_xl_optimize_gif_palette", False):
                try:
                    import av
                except ImportError:
                    ...
                    run_pip(
                        ...
                        "hotshot_xl GIF palette optimization requirement: imageio[pyav]",
                    )
                imageio.imwrite(
            ...
            if shared.opts.data.get("hotshot_xl_optimize_gif_gifsicle", False):
    ...
    def _optimize_gif(self, video_path: str):
        try:
            ...
            run_pip(
                "install pygifsicle",
                "hotshot-xl GIF optimization requirement: pygifsicle",
            )
            import pygifsicle
        finally:
            try:
    ...
    def _save_txt(
            self, params: HotshotXLParams, video_path: str, res: Processed, i: int
    ):
        res.images[i].info["temporal_layers"] = params.model
